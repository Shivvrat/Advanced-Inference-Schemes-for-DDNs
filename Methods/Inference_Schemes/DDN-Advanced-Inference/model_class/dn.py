...
class DependencyNetwork:
    def __init__(self, input_dim, output_dim, device, criterion, cfg):
        ...
        """
        self.nns = [NN(input_dim - 1, 1, device) for _ in range(output_dim)]
        self.optimizers = [
            SGD(
                each_model.parameters(),
                lr=cfg.JOINT_LEARNING.LEARNING_RATE,
    ...
    def forward_train(
        self, cnn_predictions, true_label, train, action="average", joint=False
    ):
        """
        ...
        for each_true_label_index, this_nn in enumerate(self.nns):
            ...
            if each_true_label_index == 0:
                ...
                input_to_lr = torch.cat(
                    (
                        true_label[:, :each_true_label_index],
                        true_label[:, each_true_label_index + 1 :],
            ...
            outputs[:, each_true_label_index] = this_output.reshape(
                (this_output.shape[0],)
            )
    ...
    def forward_sampling(
        self, x, cnn_predictions, var_sequence, device, sample_probs=False
    ):
        """
        ...
        for each_true_label_index in var_sequence:
            if each_true_label_index == 0:
                ...
                input_to_lr = torch.cat(
                    (
                        x[:, :each_true_label_index],
                        x[:, each_true_label_index + 1 :],
                        cnn_predictions,
                    ),
                    1,
                )
            ...
            if sample_probs:
                ...
                x[:, each_true_label_index] = this_sample.reshape(
                    (this_sample.shape[0],)
                )
            output_probs[:, each_true_label_index] = this_output.reshape(
                (this_output.shape[0],)
            )
