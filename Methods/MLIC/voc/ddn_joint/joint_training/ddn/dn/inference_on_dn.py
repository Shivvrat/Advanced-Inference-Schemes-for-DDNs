...
import torch
import wandb
from ddn.dn.inference.gibbs_sampling import sample
from ddn_utils import calculate_evaluation_metrics, create_directory, get_cuda_status_as_device, init_data, \
...
def get_model(cfg):
    ...
    with open(cfg.MODEL.SAVE_MODEL_PATH + "trained_dn_lr_model.pkl", 'rb') as file:
...
def gibbs_sampling(valid_actual_output, valid_predictions, models, num_samples, model_save_path, batch_size, logger,
    ...
    for threshold in [0.1, 0.2, 0.3, 0.5]:
        ...
        results_dir = f'{model_save_path}/cnn/'
        ...
        if threshold == 0.5:
            try:
                wandb.alert(title=f"Metrics for CNN",
                            text=f"{eval_metrics}")
            except:
    ...
    try:
        ...
        print("Cannot change spawn method for torch")
    ...
    # exit()
    val_loader, this_valid_x, this_valid_y = init_data(valid_actual_output, valid_predictions, batch_size, device, cfg,
                                                       train=False)
    ...
    if not debug:
        ...
        for threshold in [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
            ...
            if threshold == 0.5:
                try:
                    wandb.alert(title=f"Metrics for DDN - {cfg.JOINT_LEARNING.DN_TYPE}",
                                text=f"{eval_metrics}")
                except:
                    logger.info("Wandb not installed")
            print_and_save_eval_metrics(eval_metrics, output_filename, logger)
