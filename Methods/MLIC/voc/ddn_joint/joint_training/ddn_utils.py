...
def init_data(labels, predictions, batch_size, device, cfg, train=False):
    from torch.utils.data import DataLoader
    this_y = labels
    this_x = predictions
    this_valid_y = get_torch_float_tensor(this_y, device)
    this_valid_x = get_torch_float_tensor(this_x, device)
    val_set = Custom_Dataset(this_x, this_y)
...
def get_data_from_markov_network_train_data(location, logger):
    ...
    actual_labels, CNN_predictions = data[:,
                                          :data.shape[1] // 2], data[:, data.shape[1] // 2:]
...
def get_data_from_acar_output_csv(train_CNN_predictions_and_true_labels_location):
    ...
    """
    import numpy as np
    test_predictions_and_actual_output = np.genfromtxt(
        train_CNN_predictions_and_true_labels_location, delimiter=',')
    num_true_labels = test_predictions_and_actual_output.shape[1] // 2
    test_actual_labels = test_predictions_and_actual_output[:,
                                                            :num_true_labels]
    test_CNN_predictions = test_predictions_and_actual_output[:,
                                                              num_true_labels:]
...
def get_truth_values_given_index(variables, index_value, cardinalities):
    ...
    while number < len(variables):
        truth_table_value.append(
            int(index_value // numpy.prod(cardinalities[number + 1:])))
...
def calculate_evaluation_metrics(y_true, y_pred_probability, threshold):
    ...
    y_pred_probability = y_pred_probability.reshape(
        y_pred_probability.shape[0], -1)
...
def print_and_save_eval_metrics(metrics, output_filename, logger):
    ...
    logger.info("Label Ranking Average Precision Score : "
                + str(label_ranking_average_precision_score))
    ...
    import pandas as pd
    (pd.DataFrame.from_dict(data=eval_dict, orient='index').to_csv(
...
def import_edge_list(edge_list_file_path):
    ...
    edge_list = genfromtxt(edge_list_file_path,
