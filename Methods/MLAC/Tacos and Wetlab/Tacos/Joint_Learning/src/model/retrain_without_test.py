...
tf.app.flags.DEFINE_string('output_graph', '/tmp/debug.pb',
...
tf.app.flags.DEFINE_integer('how_many_training_steps', 300,
...
tf.app.flags.DEFINE_string('model_dir', 'data/pre-trained_model/model_dir',
...
# Saved CNN paths
tf.app.flags.DEFINE_string('weights_and_biases_path', "",
                           """Where to load the cnn weights and biases from""")
# Saved DN model name
tf.app.flags.DEFINE_string('model_name', "",
                           """Should we load the dn or nn""")

tf.app.flags.DEFINE_string('date', "",
                           """Date time when this code was started""")
...
def add_custom_gradient_to_final_value(x: tf.Tensor, gradients_from_dn: tf.Tensor) -> tf.Tensor:
    """
...
def add_final_loss(class_count, final_tensor_name, bottleneck_tensor, dn_models):
    with tf.name_scope('input'):
        gradients_for_CNN = tf.placeholder(tf.float32,
                                           [None, class_count],
                                           name='GradientsDN')
        bottleneck_input = tf.placeholder_with_default(
                bottleneck_tensor, shape=[None, BOTTLENECK_TENSOR_SIZE],
    ...
    output_with_gradient_from_dn = add_custom_gradient_to_final_value(logits, gradients_for_CNN)
    ...
    with tf.name_scope('train'):
        ...
        train_step = tf.train.GradientDescentOptimizer(FLAGS.learning_rate).minimize(
            cross_entropy_mean)
    return (
        train_step, cross_entropy_mean, bottleneck_input, ground_truth_input, predictions_of_CNN, gradients_for_CNN)
...
def train():
    ...
    wandb.init(project=f"Joint Learning Tacos")
    cfg = load_cfgs(None)
    ...
    if class_count == 0:
        print(f'No valid labels inside file {ALL_LABELS_FILE} that should contain all possible labels (= classes).')
        return -1
    if class_count == 1:
        print(f"Only one valid label found inside {ALL_LABELS_FILE} - multiple classes are needed for classification.")
        return -1
    ...
    (train_step, cross_entropy, bottleneck_input, ground_truth_input,
     predictions_of_CNN, gradients_for_CNN) = add_final_loss(class_count,
                                                             FLAGS.final_tensor_name,
                                                             bottleneck_tensor, dn_models)
    ...
    while i < FLAGS.how_many_training_steps and not stop:
        ...
        gradients_from_dn = train_dn(train_bottlenecks, sess, bottleneck_tensor, train_ground_truth, dn_models)
        # if FLAGS.DEBUG:
        #     print(gradients_from_dn)
        # print()
        train_summary, _ = sess.run([merged_final, train_step],
                                    feed_dict={gradients_for_CNN: gradients_from_dn,
