...
FINAL_BIASES, FINAL_WEIGHTS = get_values_of_weights_and_biases(
    weights_and_biases_path)
    ...
    with tf.name_scope('accuracy'):
        with tf.name_scope('correct_prediction'):
            ...
            correct_prediction = tf.equal(
                tf.round(result_tensor), ground_truth_tensor)
        with tf.name_scope('accuracy'):
...
def train():
    ...
    evaluation_step = add_evaluation_step(
        predictions_of_CNN, ground_truth_input)
    ...
    if not FLAGS.DEBUG:
        ...
        print(
            f"Saving the dependency network at {dn_model_save_location_torch}")
